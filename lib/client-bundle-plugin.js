...
export function getDependencies(clientOutput, item) {
  ...
  for (const fileName of crawlDependencies) {
    ...
    // If chunk is not found, it might be a referenced asset, not a JS chunk.
    // We should still add its referenced files if it's a chunk.
    ...
    // Always add referenced files, regardless of chunk type.
    if (chunk && chunk.referencedFiles) {
...
export default function (inputOptions, outputOptions, resolveFileUrl) {
  ...
  return {
    ...
    resolveImportMeta(property, { moduleId, format }) {
      ...
      // Find the client entry. It must be a 'chunk' and its facadeModuleId must match the source ID.
      const clientEntry = clientOutput.find(
        (item) =>
          item.type === 'chunk' &&
          item.facadeModuleId &&
          path.normalize(item.facadeModuleId) === id,
      );

      if (!clientEntry) {
        throw new Error(
          `Client bundle output missing expected entry chunk for source ID: ${id}. This indicates a bundling issue where the entry point was not correctly generated as a facade chunk.`,
        );
      }

      if (property.startsWith(entryPathPlaceholder)) {
        // Return the generated file name (relative path) for the entry point.
        // clientEntry.fileName should always be the relative output path for a chunk.
      ...
      return (
        ...
        dependencies
          .map((item) => {
            ...
            if (!entry) {
              throw new Error(
                `Client bundle output missing expected dependency: ${item}`,
              );
            }
            return resolveFileUrl({
